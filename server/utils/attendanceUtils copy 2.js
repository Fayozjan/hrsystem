...
function getCachedShift(event) {
  const key = `${event.user_id}_${event.event_time_string}`;
...
export function processEvents(events, tripsMap = {}) {
  ...
  for (const event of events) {
    if (!groupedByUser[event.user_id]) groupedByUser[event.user_id] = [];
    groupedByUser[event.user_id].push(event);
  }
  ...
  for (const userId in groupedByUser) {
    ...
    if (currentEvents.length > 0) {
      sessions.push(
        createSession(currentSession, currentEvents, tripsMap[userId] || [])
      );
    }
...
function addMinutesToDuration(durationStr, minutesToAdd) {
...
function createSession(session, events, tripsForUser) {
  ...
  if (session.type === "normal") {
    ...
    if (leaveRequest) {
      base.hasTrip = true;
      base.companyPaid = leaveRequest.is_company_paid;

      if (leaveRequest.is_company_paid) {
        const leaveStart = DateTime.fromFormat(
  ...
  if (leaveRequest) {
    ...
    if (leaveRequest.is_company_paid) {
      base.workDuration = addMinutesToDuration(base.workDuration, 8 * 60);
    }

    base.events.push({
      event_type: "trip",
      description: leaveRequest.is_company_paid
        ? "Командировка за счет компании"
        : "Командировка",
      company_paid: leaveRequest.is_company_paid,
    });
  }
...
/*
// Функция для определения смены для события
function determineShift(eventTime, event) {
  ...
  // Вторая смена может заканчиваться на следующий день
  ...
  if (event.event_type === "entry") {
    const diffToFirst = Math.abs(
      eventTime.diff(firstShiftStart, "minutes").minutes
    );
    const diffToSecond = Math.abs(
      eventTime.diff(secondShiftStart, "minutes").minutes
    );

    return diffToFirst <= diffToSecond ? "first" : "second";
  }

  if (event.event_type === "exit") {
    const diffToFirst = Math.abs(
      eventTime.diff(firstShiftEnd, "minutes").minutes
    );
    const diffToSecond = Math.abs(
      eventTime.diff(secondShiftEnd, "minutes").minutes
    );
