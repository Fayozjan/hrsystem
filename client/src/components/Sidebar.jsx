import { useEffect, useState } from "react";
import { NavLink, useLocation } from "react-router-dom";
import { useTranslation } from "react-i18next";

import { useAuthStore } from "../stores/authStore";

import { Icons } from "../icons/sidebarIcons";
import Profile from "./Profile";
import ProfileSettings from "./ProfileSettings";
import CenterModal from "./CenterModal";
import OverlaySidebar from "./OverlaySidebar";

import styles from "./Sidebar.module.scss";

const iconMap = {
  dashboard: Icons.dashboard,
  hr: Icons.hr,
  employees: Icons.employees,
  "time-off": Icons.timeOff,
  branches: Icons.branches,
  departments: Icons.departments,
  positions: Icons.positions,
  "work-schedules": Icons.workSchedules,
  holidays: Icons.holidays,
  reports: Icons.reports,
  attendance: Icons.attendance,
  "late-employees": Icons.lateEmployees,
  timesheet: Icons.timesheet,
  "face-passes": Icons.facePasses,
  manufacturing: Icons.manufacturing,
  "manufacturing-orders": Icons.faceDevices,
  settings: Icons.settings,
  users: Icons.users,
  doors: Icons.doors,
  "face-devices": Icons.faceDevices,
  "telegram-bots": Icons.telegramBots,
};

const Sidebar = ({ menuData }) => {
  const { userSettings, setSidebarState, logout } = useAuthStore();
  const [openItems, setOpenItems] = useState({});
  const [hoveredItem, setHoveredItem] = useState(null);
  const location = useLocation();
  const [profileSettings, setProfileSettings] = useState(false);

  const isOpen = userSettings.sidebar;

  const [showModal, setShowModal] = useState(false);
  const { i18n, t } = useTranslation();

  useEffect(() => {
    if (userSettings?.language) {
      i18n.changeLanguage(userSettings.language);
    }
  }, [userSettings, i18n]);

  const toggleSidebar = () => {
    setSidebarState(!isOpen);
  };

  const toggleItem = (id) => {
    setOpenItems((prev) => {
      const newOpen = { ...prev };
      const openIds = Object.keys(newOpen).filter((key) => newOpen[key]);

      if (newOpen[id]) {
        // если уже открыт — закрываем
        newOpen[id] = false;
      } else {
        // если открываем новый
        if (openIds.length >= 2) {
          // закрываем самый первый открытый
          newOpen[openIds[0]] = false;
        }
        newOpen[id] = true;
      }

      return newOpen;
    });
  };

  const renderMenu = (items, type = "full") => {
    return items.map((item) => {
      if (!item.permissions?.view) return null;

      const Icon = iconMap[item.name];
      const hasChildren = item.children?.length > 0;
      const isOpen = openItems[item.id];
      const isActive =
        location.pathname === item.path ||
        location.pathname.startsWith(item.path + "/") ||
        (item.children?.some((child) =>
          location.pathname.startsWith(child.path)
        ) ??
          false);

      return (
        <li
          key={item.id}
          onMouseEnter={() => setHoveredItem(item.id)}
          onMouseLeave={() => setHoveredItem(null)}
          className={[
            styles.menu,
            isActive ? styles.activeMenu : "",
            isOpen ? styles.openSubmenu : "",
            hasChildren ? styles.menuItemHasChildren : "",
          ].join(" ")}
        >
          <NavLink
            to={item.path || "#"}
            onClick={(e) => {
              if (hasChildren) {
                if (type === "full") {
                  e.preventDefault();
                  toggleItem(item.id);
                } else {
                  e.preventDefault();
                }
              }
            }}
          >
            {Icon && Icon}
            {type !== "mini" && <span>{t(item.name)}</span>}
          </NavLink>

          {type === "mini" && hasChildren && (
            <div className={styles.miniPopupWrapper}>
              <p>{t(item.name)}</p>
              <ul className={styles.miniPopup}>
                {item.children.map((child) => (
                  <li key={child.id}>
                    <NavLink
                      to={child.path}
                      className={({ isActive }) =>
                        isActive ? styles.active : ""
                      }
                    >
                      {t(child.name)}
                    </NavLink>
                  </li>
                ))}
              </ul>
            </div>
          )}

          {type === "mini" && !hasChildren && (
            <div className={styles.miniPopupWrapper}>
              <NavLink to={item.path || "#"}>{t(item.name)}</NavLink>
            </div>
          )}

          {hasChildren && type !== "mini" && (
            <ul
              className={[
                styles.submenuWrapper,
                isOpen ? styles.show : "",
              ].join(" ")}
            >
              {renderMenu(item.children, type)}
            </ul>
          )}
        </li>
      );
    });
  };

  useEffect(() => {
    const newOpenItems = {};

    const checkTree = (items) => {
      items.forEach((item) => {
        const hasChildren = item.children?.length > 0;

        const isActive =
          location.pathname === item.path ||
          location.pathname.startsWith(item.path + "/") ||
          (item.children?.some((child) =>
            location.pathname.startsWith(child.path)
          ) ??
            false);

        if (isActive && hasChildren) {
          newOpenItems[item.id] = true;
        }

        if (hasChildren) {
          checkTree(item.children);
        }
      });
    };

    checkTree(menuData);
    setOpenItems(newOpenItems);
  }, [location.pathname, menuData]);

  const fullMenu = renderMenu(menuData, "full");
  const miniMenu = renderMenu(menuData, "mini");

  const handleShowModal = () => {
    setShowModal(true);
  };

  return (
    <>
      <div
        className={`${styles.sidebar} ${isOpen ? styles.open : styles.closed}`}
      >
        <div className={styles.sidebarTop}>
          {isOpen ? (
            <img src="/ilm.webp" alt="Logo" className={styles.logo} />
          ) : (
            <img src="/ilm-icon.webp" alt="Logo" className={styles.logoIcon} />
          )}

          <svg
            className={styles.toggle}
            onClick={() => toggleSidebar()}
            xmlns="http://www.w3.org/2000/svg"
            width="200"
            height="200"
            viewBox="0 0 24 24"
          >
            <path
              fill="#000000"
              fill-rule="evenodd"
              d="M6.416 4.767a2.65 2.65 0 0 0-2.65 2.65v8.832a2.65 2.65 0 0 0 2.65 2.65h1.461V4.767h-1.46Zm0-1.767A4.416 4.416 0 0 0 2 7.416v8.833a4.416 4.416 0 0 0 4.416 4.417h11.168A4.416 4.416 0 0 0 22 16.248V7.416A4.416 4.416 0 0 0 17.584 3zm3.228 1.767v14.132h7.94a2.65 2.65 0 0 0 2.65-2.65V7.416a2.65 2.65 0 0 0-2.65-2.65h-7.94Z"
              clip-rule="evenodd"
            />
          </svg>
        </div>
        <ul className={styles.menuWrapper}>{isOpen ? fullMenu : miniMenu}</ul>

        {isOpen ? (
          <Profile />
        ) : (
          <div className={styles.footer}>
            <svg
              className={styles.logout}
              onClick={() => setProfileSettings(true)}
              xmlns="http://www.w3.org/2000/svg"
              width="200"
              height="200"
              viewBox="0 0 1024 1024"
            >
              <path d="m960.496 415.056l-82.129-18.224c-6.4-20.48-14.784-40.08-24.4-58.927l44.431-74.032c16.592-26.512 24.976-65.52 0-90.512l-45.28-45.248c-24.976-24.992-67.151-20.496-92.623-2.832l-72.032 45.887c-18.689-9.696-38.225-18-58.529-24.56l-18.431-83.12C605.999 33.009 579.343 0 543.999 0h-64c-35.344 0-57.008 33.504-64 64l-20.528 82.128c-21.68 6.912-42.496 15.744-62.336 26.208l-73.84-47.024c-25.456-17.664-67.648-22.16-92.624 2.832l-45.264 45.248c-24.992 25.008-16.608 64 0 90.512l46.752 77.92c-8.767 17.664-16.544 35.936-22.544 55.024l-82.112 18.224C33.007 420.56 0 447.216 0 482.56v64c0 35.344 33.504 57.008 64 64l83.152 20.784c5.745 17.632 12.928 34.56 21.056 50.976l-46.8 78c-16.591 26.496-24.975 65.504 0 90.496l45.28 45.248c24.976 25.008 67.152 20.496 92.624 2.847l74-47.152c19.952 10.528 40.88 19.44 62.704 26.337L416.495 960c7.008 30.496 28.656 64 64 64h64c35.344 0 62-33.007 67.504-63.504l18.464-83.343c20.096-6.496 39.376-14.689 57.84-24.257l72.192 46c25.472 17.664 67.664 22.16 92.624-2.848L898.4 850.8c24.976-25.008 16.592-64 0-90.496l-44.463-74.128c8.944-17.568 16.688-35.84 22.912-54.848L960 610.56c30.496-7.008 64-28.656 64-64v-64c0-35.344-32.992-62-63.504-67.504zm-.465 126.992c-2.72 1.952-7.842 4.635-14.338 6.139l-118.656 29.631l-11.008 33.632c-4.975 15.153-11.407 30.529-19.119 45.712l-16.064 31.569l62.688 104.528c4 6.4 5.872 12.127 6.432 15.503l-42.096 42.033c-4.064-1.28-8.688-2.945-10.912-4.464l-105.344-67.184l-32.752 16.945c-15.776 8.192-31.969 14.976-48.097 20.192l-34.88 11.28l-26.368 119.12c-1.216 6.368-4.624 11.504-6.96 13.344h-57.6c-1.951-2.72-4.623-7.84-6.112-14.32L449.39 827.9l-34.095-10.817c-17.569-5.536-35.088-12.912-52.144-21.904l-32.912-17.376l-105.36 67.152c-4.304 2.912-8.912 4.56-13.088 4.56l-41.968-40.847c.56-3.311 2.304-8.783 5.792-14.367l65.456-109.056l-15.568-31.344c-7.264-14.784-13.024-28.656-17.504-42.4l-10.992-33.664L79.518 548.46c-7.392-1.68-12.736-4.432-15.52-6.4v-59.504a.32.32 0 0 0 .145.032c1.072 0 6.336-3.745 10.72-4.544l120.72-26.737l11.087-35.28c4.512-14.368 10.672-29.344 18.816-45.775l15.568-31.36l-64.767-107.92c-4.016-6.432-5.872-12.16-6.432-15.52l42.08-42.065c4.08 1.312 8.672 2.96 10.88 4.48l107.312 68.4l32.88-17.344c16.88-8.895 34.336-16.239 51.904-21.823l34.016-10.832L478.11 79.501c1.697-7.391 4.416-12.735 6.4-15.52H544c-.433.657 3.68 6.24 4.527 10.865l26.88 121.408l34.848 11.264c16.336 5.28 32.752 12.16 48.72 20.448l32.752 17.008l103.152-65.712c4.32-2.945 8.944-4.576 13.088-4.576l42 40.816c-.56 3.328-2.32 8.816-5.808 14.416l-63.344 105.488l16.16 31.616c8.72 17.056 15.376 33.056 20.32 48.928l11.056 35.344L946.64 477.55c7.153 1.328 12.721 5.456 13.905 7.696zM512.43 319.674c-106.272 0-192.736 86.288-192.736 192.32c0 106.016 86.464 192.304 192.736 192.304s192.736-86.288 192.736-192.304c0-106.032-86.464-192.32-192.736-192.32zm-.432 320.32c-70.576 0-128-57.424-128-128c0-70.592 57.424-128 128-128c70.592 0 128 57.408 128 128c0 70.576-57.424 128-128 128z" />
            </svg>
            <svg
              className={styles.logout}
              onClick={handleShowModal}
              xmlns="http://www.w3.org/2000/svg"
              width="200"
              height="200"
              viewBox="0 0 1024 1024"
            >
              <path d="M116.832 543.664H671.28c17.696 0 32-14.336 32-32s-14.304-32-32-32H118.832l115.76-115.76c12.496-12.496 12.496-32.752 0-45.248s-32.752-12.496-45.248 0l-189.008 194l189.008 194c6.256 6.256 14.432 9.376 22.624 9.376s16.368-3.12 22.624-9.376c12.496-12.496 12.496-32.752 0-45.248zM959.664 0H415.663c-35.36 0-64 28.656-64 64v288h64.416V103.024c0-21.376 17.344-38.72 38.72-38.72h464.72c21.391 0 38.72 17.344 38.72 38.72l1.007 818.288c0 21.376-17.328 38.72-38.72 38.72h-465.71c-21.376 0-38.72-17.344-38.72-38.72V670.944l-64.416.08V960c0 35.344 28.64 64 64 64h543.984c35.36 0 64.016-28.656 64.016-64V64c-.015-35.344-28.671-64-64.015-64z" />
            </svg>
          </div>
        )}
      </div>

      <CenterModal
        isOpen={showModal}
        onClose={() => setShowModal(false)}
        onAccept={() => logout()}
        title="Вы уверены, что хотите выйти?"
      />

      <OverlaySidebar
        width="400px"
        isOpen={profileSettings}
        onClose={() => setProfileSettings(false)}
        title="Настройки профиля"
      >
        <ProfileSettings />
      </OverlaySidebar>
    </>
  );
};

export default Sidebar;
